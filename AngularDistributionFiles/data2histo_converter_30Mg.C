...
TH2F *hdsigma1VsTheta = new TH2F("hdsigma1VsTheta","d#sigma / d#omega vs #theta for 30Mg(d,p)31Mg ",180,0,180,1000,3.,997);
TH2F *hdsigma2VsTheta = new TH2F("hdsigma2VsTheta","d#sigma_{2} / d#omega vs #theta ",180,0,180,1000,3.,997);
TH2F *hdsigma3VsTheta = new TH2F("hdsigma3VsTheta","d#sigma_{3} / d#omega vs #theta ",180,0,180,1000,3.,997);

//*************************************** Read Momenta from the file ******************************************************************//

Int_t counter = 0;
Float_t theta[200], sigma1[200], sigma2[200], sigma3[200], dummy=0;

ifstream infile_1;

infile_1.open("30Mg_dp.dat"); 
//infile_1.open("Mg30PereyFrescoSpline_p.dat"); 

while(!infile_1.eof())
{
  
      infile_1 >> theta[counter] >> sigma1[counter] >> dummy >> sigma2[counter] >> dummy >> sigma3[counter];
      //infile_1 >> theta[counter] >> sigma1[counter] >> dummy >> dummy >> dummy >> dummy;
          counter++;

            if(counter>0) cout<<"counter: "<<counter<< "\t" <<theta[counter-1]<< "\t" <<sigma1[counter-1]<< "\t" <<sigma2[counter-1]<< "\t" <<sigma3[counter-1]<<endl;
	...
	for(int i=0;i<counter-1;i++){
		...
		//cout<<" i: "<<i<<"\t" <<theta[i]<< "\t" <<sigma1[i]<< "\t" <<sigma2[i]<< "\t" <<sigma3[i]<<endl;                   
  ...
  L_all->AddEntry(hdsigma2VsTheta,"50.1 keV 3/2+ state l=2","lp");
  L_all->AddEntry(hdsigma3VsTheta,"221.1 keV 3/2- state l=1","lp");
...
//0.000        487.22	      0.000 	   49.1192	    0.00 	 	 194.016 
//180.0      5.618         180.0       6.705          180.0       39.92  
}
